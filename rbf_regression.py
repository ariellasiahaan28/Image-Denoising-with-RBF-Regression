...
class RBFRegression():
    ...
    def rbf_2d(self, X, i):
        ...
        ...
        rbf_out = np.exp(((-1) * ((X - rbf_center) ** 2) @ np.ones(shape=(2,1))) / (2 * (rbf_width ** 2)))
        # ====================================================
    ...
    def predict(self, X):
        ...
        rbf_i = np.hstack([self.rbf_2d(X, i) for i in range(1, self.K+1)])
        B = np.hstack((np.ones(shape=(X.shape[0], 1)), rbf_i))
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coeff):
        ...
        #Compute rbf output for each input i
        rbf_i = np.hstack([self.rbf_2d(train_X, i) for i in range(1, self.K+1)])

        #Compute matrix B
        B = np.hstack((np.ones(shape=(train_X.shape[0], 1)), rbf_i))

        #Computer self.parameters
        self.parameters = np.linalg.inv(B.T @ B + l2_coeff * np.eye(self.K + 1)) @ B.T @ train_Y
